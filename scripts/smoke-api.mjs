...
function assertPagination(body, label) {
  ...
  assert.equal(
    typeof body.pagination.page,
    "number",
    `${label} pagination.page should be a number`,
  );
  assert.equal(
    typeof body.pagination.hasNext,
    "boolean",
    `${label} pagination.hasNext should be a boolean`,
  );
}
...
assert.equal(
  me.body.user.email,
  email,
  "me should return the authenticated user",
);
...
if (events.body.events.length > 0) {
  ...
  const comments = await request(
    `/api/events/${firstEventId}/comments?page=1`,
    { headers: authHeaders },
  );
...
assert.equal(
  ...
  401,
  "invalid login should be rejected with 401",
);

...
import { softDeleteEventComment, updateEventComment } from "@/services/comments/comment-service";
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const comment = await updateEventComment(commentId, user, String(body.text ?? ""), eventId);

    if (!comment) {
      return NextResponse.json({ error: "Коментарът не е намерен." }, { status: 404 });
    }
